...
class Machine(Thread):
    ...
    def working_piece_to_finished(self):
        ...
        if order_finished:            
            ...
            print(order_finished, flush=True)
            publish('order_event_exchange', 'machine_response_queue', order_finished)

        self.thread_session.commit()
        self.thread_session.flush()

    def add_pieces_to_queue(self, pieces):
        print('adding pieces', flush=True)
        for piece in pieces:
            print(piece)
    ...
    def add_piece_to_queue(self, piece):
        ...
        print("Adding piece to queue: {}".format(piece.ref), flush=True)
